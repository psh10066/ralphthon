...
import { getAIConfig, visionCompletion, chatCompletion } from "@/lib/ai";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = SYSTEM_PROMPT + previousContext;
    ...
    if (hasImage) {
      ...
      responseText = await visionCompletion({ config, system: systemPrompt, userContent });
    } else {
      const typeLabel = inputType === "link" ? "링크" : inputType === "memo" ? "메모" : "글";
      const userMessage = `[${typeLabel} 입력]\n\n${text}\n\n이 내용에서 취향과 패턴을 읽어주세요. JSON만 출력하세요.`;
